...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .hero {
      background: linear-gradient(135deg, #0c3557, #1867b0 65%, #2d8dce);
      color: #fff;
      padding: 82px 0 72px;
    }
    ...
    .offer {
      ...
      border-radius: 16px;
      padding: 28px;
      box-shadow: 0 18px 45px #061b2e59;
    }
...
<body>
  <header>
    <div class="wrap nav">
      <div class="brand">Next <span>Chair</span></div>
      <div class="state">${state} · Regional launch offer</div>
    </div>
  </header>
  ...
  <main>
    <section class="hero">
      <div class="wrap hero-grid">
        <div>
          <div class="eyebrow">For ${state} small businesses</div>
          <h1>Your next customer is already looking.</h1>
          <p>${description}</p>
          <a class="button" href="#contact">Claim the $500 launch offer</a>
        </div>

        <aside class="offer">
          <h2>A practical launch package</h2>
          <div class="price">$500 <small>one-time launch</small></div>
          <ul>
            <li>A focused, professional website</li>
            <li>Clear offer and contact path</li>
            <li>Mobile-ready design</li>
            <li>Lead form delivered to your inbox</li>
            <li>Optional support for $125/month</li>
          </ul>
        </aside>
      </div>
    </section>
    ...
    <section class="section tint">
      <div class="wrap">
        <h2>Built to make the next step easy.</h2>
        <p class="lead">
          Next Chair helps local owners establish a credible online front door,
          so prospects can understand what you do and get in touch without friction.
        </p>

        <div class="grid">
          <article class="card">
            <h3>Clear positioning</h3>
            <p>We turn your service and value into copy customers can quickly understand.</p>
          </article>

          <article class="card">
            <h3>Designed for action</h3>
            <p>Every page points visitors toward a call, email, or a simple inquiry.</p>
          </article>

          <article class="card">
            <h3>Support if you want it</h3>
            <p>Keep your site current with optional ongoing support at $125 per month.</p>
          </article>
        </div>
      </div>
    </section>
    ...
    <section class="section" id="contact">
      <div class="wrap form-box">
        <div>
          <div class="eyebrow">Start your ${short} launch</div>
          <h2>Tell us about your business.</h2>
          <p>
            Share your details and we will follow up about the $500 launch package.
            Prefer to talk now? Call
            <a class="phone" href="tel:+12024231445">(202) 423-1445</a>.
          </p>
        </div>
        ...
        <form class="form" id="lead-form">
          ...
          <div>
            ...
            <textarea id="message" name="message" placeholder="A brief description is perfect."></textarea>
          </div>

          <button class="button" type="submit">Request my launch offer</button>
          <p class="note">Your email is required so we can respond.</p>
        </form>
      </div>
    </section>
  </main>
  ...
  <footer>
    <div class="wrap">© ${new Date().getFullYear()} Next Chair · ${state}</div>
  </footer>
  ...
  <script>
    ...
    form.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        button.textContent = 'Request my launch offer';
      }
    });
  </script>
</body>
...
async function handleLead(request, env) {
  ...
  const region =
    hostname === 'va.nextchair.com.businessdatagurus.com' ||
    request.headers.get('Referer')?.includes('/next-chair-va')
      ? 'Virginia'
      : 'Maryland';
  ...
  const notificationEmail =
    env.NOTIFICATION_EMAIL || 'hello@businessdatagurus.com';
...
Message: ${message || 'Not provided'}`;
  ...
  const replySubject = encodeURIComponent('Re: Your Next Chair launch offer');
          ...
          <table role="presentation" width="600" cellspacing="0" cellpadding="0" border="0" style="width:100%;max-width:600px;background:#ffffff;border-radius:14px;overflow:hidden;">
            <tr>
              <td style="padding:24px 28px;background:#0c3557;color:#ffffff;">
                ...
                <div style="padding-top:5px;font-size:13px;line-height:1.4;color:#b9e1ff;">
                  New ${escapeHtml(region)} launch-offer lead
                </div>
              </td>
            </tr>
            <tr>
              <td style="padding:28px;">
                <h1 style="margin:0 0 18px;font-size:22px;line-height:1.3;color:#13202a;">
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="border-collapse:collapse;">
                  ...
                  <tr>
                    ...
                    <td style="padding:11px 0;border-bottom:1px solid #dce9f3;font-size:14px;">
                      <a href="mailto:${safeEmail}" style="color:#1867b0;text-decoration:none;">${safeEmail}</a>
                    </td>
                  </tr>
                  <tr>
                ...
                <div style="padding-top:24px;">
                  <a href="mailto:${safeEmail}?subject=${replySubject}" style="display:inline-block;padding:12px 18px;background:#f2b441;border-radius:7px;color:#13202a;font-size:14px;font-weight:700;text-decoration:none;">
                    Reply to ${safeName}
                  </a>
                </div>

                <p style="margin:24px 0 0;font-size:12px;line-height:1.5;color:#5b6871;">
                  This lead was submitted through the Next Chair ${escapeHtml(region)} landing page.
                </p>
              </td>
            </tr>
            <tr>
...
export default {
  async fetch(request, env) {
    ...
    if (
      request.method === 'GET' &&
      url.pathname === '/' &&
      hostname === 'md.nextchair.com.businessdatagurus.com'
    ) {
    ...
    if (
      request.method === 'GET' &&
      (url.pathname === '/next-chair-md' || url.pathname === '/next-chair-md/')
    ) {
      return new Response(page('md'), {
        headers: {
          'content-type': 'text/html; charset=UTF-8'
    ...
    if (
      request.method === 'GET' &&
      (url.pathname === '/next-chair-va' || url.pathname === '/next-chair-va/')
    ) {
      return new Response(page('va'), {
        headers: {
          'content-type': 'text/html; charset=UTF-8'
